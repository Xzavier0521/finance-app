package cn.zhishush.finance.web.controller.oauth.user;
...
import cn.zhishush.finance.api.model.vo.userAccount.FinanceUserAccountVo;
...
import cn.zhishush.finance.core.common.util.ArithUtil;
...
public class UserInfoApi {
    ...
    public ResponseResult<UserParentInfoDetailVO> queryParentUserInfo() {
        ResponseResult<UserParentInfoDetailVO> response;
        log.info("[开始查询用户的上级用户信息]");
        try {
            UserInfo userInfo = UserInfoConverter.convert(jwtService.getUserInfo());
            UserParentInfoDetailVO userParentInfoDetailVO = userInfoQueryService
                .queryParentUserInfo(userInfo);
            response = ResponseResultUtils.success(userParentInfoDetailVO);
        } catch (final Exception e) {
