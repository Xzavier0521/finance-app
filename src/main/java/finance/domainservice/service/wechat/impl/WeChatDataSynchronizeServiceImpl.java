 ...
 * 
 * @author lili
 * @version $Id: WeChatDataSynchronizeServiceImpl.java, v0.1 2018/10/23 10:24 PM
 *          lili Exp $
 */
@Slf4j
...
public class WeChatDataSynchronizeServiceImpl implements WeChatDataSynchronizeService {
	...
	public void process() {
		...
		if (Objects.nonNull(response)) {
			...
			openIds = response.getData().getOpenid().toArray(new String[response.getData().getOpenid().size()]);
			...
			while (StringUtils.isNotBlank(response.getNext_openid())) {
				response = weiXinUserInfoQueryClient.queryUserList(accessToken, response.getNext_openid());
	...
	private void sendMessage(String openId) {
		try {
			...
			if (Objects.nonNull(weiXinUserInfoDetail)) {
				...
				weiXinTemplateMessageSendService.send(null, thirdAccountInfo, weiXinMessageTemplate, parameters);
			} else {
	...
	private void processBatch(int pageNum, int totalPage, List<String> dateList) {
		...
		inviteOpenInfoRepository.batchUpdateStatusByOpenId(dateSubList, ConcernStatusEnum.SUBSCRIBE);
	}
}
