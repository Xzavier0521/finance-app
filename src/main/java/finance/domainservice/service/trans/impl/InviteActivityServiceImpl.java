...
public class InviteActivityServiceImpl implements InviteActivityService {
	private static final Logger logger = LoggerFactory.getLogger(InviteActivityServiceImpl.class);
	@Resource
	...
	public void stepRewards(UserInfoDO userInfo, LoginParamDto paramDto) {
		if (!"1".equals(stepRedEnvelopeSwitch)) {
			// 阶梯红包活动开关关闭时，结束活动
			logger.info(LogUtil.getFormatLog("step.red.envelope.switch=" + stepRedEnvelopeSwitch, "阶梯红包活动开关关闭，结束活动"));
			return;
		}
		// 总奖池扣除对应金额
		// 加锁（初始化id=1）
		StepRewardsAmountDO rewardsPoolAmount = stepRewardsAmountMapper.selectByPrimaryKeyForUpdate(1L);
		...
		if (CollectionUtils.isNotEmpty(grandInviteInfoList)) {
			...
			UserInfoDO grandUserInfo = userInfoMapper.selectByPrimaryKey(grandInviteInfo.getParentUserId());
			accountService.charge("新增二级用户奖励", grandUserInfo.getId(), secondInviteAmount, grandUserInfo.getMobileNum());
			// 佣金提醒
			...
			updateRewardsPoolAmount.setCurrentAmount(rewardsPoolAmount.getCurrentAmount().subtract(secondInviteAmount));
		...
		updateRewardsPoolAmount.setCurrentAmount(
				rewardsPoolAmount.getCurrentAmount().subtract(rewardAmount).subtract(secondInviteAmount));
		stepRewardsAmountMapper.updateByPrimaryKeySelective(updateRewardsPoolAmount);

		// 充值奖励金额
		UserInfoDO parentUserInfo = userInfoMapper.selectByPrimaryKey(userInviteInfo.getParentUserId());
		accountService.charge("阶梯红包奖励", userInviteInfo.getParentUserId(), rewardAmount, parentUserInfo.getMobileNum());
		// 佣金提醒
		registerSendMessageService.sendRewardMessage(userInviteInfo.getParentUserId(), rewardAmount);
	...
	private BigDecimal queryRewardAmount(Integer inviteNum) {
		...
		while (iter.hasNext()) {
			...
			BigDecimal previousValue = linkedMap.previousKey(key) == null
	...
	private BigDecimal queryEndAmount() {
		...
		BigDecimal penultValue = (BigDecimal) linkedMap.get(linkedMap.previousKey(linkedMap.lastKey()));
	...
	public Map<String, Object> getFixedAmountEntry() {
		...
		/** 2.根据userId 查询活动信息 **/
		Page<ActivityFixedAmountMainDO> financeActivityFixedAmountMainPage = new Page<ActivityFixedAmountMainDO>(1, 1l);
	...
	private FixedAmountPageVO getFixedAmountEntryPageInfo(FixedAmountPageDto fixedAmountPageDto, Long userId) {
		int pageSize = fixedAmountPageDto.getPageSize();
		Long pageNum = fixedAmountPageDto.getPageNum();
		Page<ActivityFixedAmountMainDO> financeActivityFixedAmountMainPage = new Page<>(pageSize, pageNum);
		...
		if (total > 0) {
			...
			activityFixedAmountMainDOS
					.forEach(activityFixedAmountMainDO -> userIds.add(activityFixedAmountMainDO.getUserId()));
			List<UserInfoDO> userInfoList = userIds.isEmpty()
					? new ArrayList<>()
	...
	private FixedAmountPageVO getUserDetailActivityInfo(Long userId, Long activityId) {
		...
		fixedAmountPageVO.setRedPacketTotalAmount(activityTotalAmount);
		fixedAmountPageVO.setRedPacketSponsorAmount(activitySponsorGetAmount);
		fixedAmountPageVO.setRemainingAmount(
				activityTotalAmount.multiply(activitySponsorGetAmount).multiply(activityOtherPeopleDivideAmount));
	...
	public ResponseResult<FixedAmountPageVO> joinFixedAmountActivity() {
		...
		ActivityFixedAmountMainDO activityFixedAmountMainDO = financeActivityFixedAmountMainMapper
				.selectByUserId(userId, financeActivityFixedAmountMainPage);
		if (activityFixedAmountMainDO != null && activityFixedAmountMainDO.getState().intValue() == 0) {
		...
		/** 4.生成6个随机红包 **/
		List<BigDecimal> returnList = divideRedPacket(redPacketAmount, fixAmountRedPacketNum + 1, 0.01);
	...
	@Override
	@Transactional
	public Map<String, Object> handleFixedAmountActivity(UserInfoDO userInfo, LoginParamDto paramDto) {
		...
		if (beforUpdate < 5 && beforUpdate == 4) {
			...
			// 充值
			accountService.charge("固定金额活动奖励", parentUser.getId(), activityFixedAmountMainDO.getTotalAmount(),
					parentUser.getMobileNum());
