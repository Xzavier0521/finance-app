...
import cn.zhishush.finance.domainservice.service.login.impl.LoginServiceImpl;
import cn.zhishush.finance.core.dal.dao.log.ThirdAccountActionLogDAO;
...
import cn.zhishush.finance.core.common.enums.ThirdLoginAction;
...
import cn.zhishush.finance.core.dal.dataobject.third.ThirdAccountActionLogDO;
...
public class ThirdBindServiceImpl implements ThirdBindService {
	...
	@Resource
	private ThirdAccountActionLogDAO thirdAccountActionLogMapper;
	@Resource
	private WechatClient wechatClient;

	@Override
	public ResponseResult<Boolean> bindUser(Long userId, String openId, String channel, String wechatPubName) {
		// 保存绑定信息
		...
		FinanceThirdAccountInfo accountInfo = thirdAccountInfoMapper.selectOneBySelective(queryParam);
		...
		// 记录绑定日志
		ThirdAccountActionLogDO actionLog = new ThirdAccountActionLogDO();
	...
	public String queryOpenInfo(String channel, String code, String openId) {
		...
		switch (channel) {
			case "wechat" :
				WechatOpenInfoDto openInfo = wechatClient.getOpenInfo(code, Constant.THIRD_PARD_WECHAT);
				...
				break;
			case "wechatPub" :
				WechatOpenInfoDto openInfoPub = wechatClient.getOpenInfo(code, Constant.THIRD_PARD_WECHATPUB);
