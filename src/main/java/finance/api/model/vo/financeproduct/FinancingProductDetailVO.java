...
public class FinancingProductDetailVO {
    ...
    public void setTerminalBonus(BigDecimal terminalBonus, Integer amountType) {
        if (AmountType.amount.getCode().equals(amountType)) {
            this.terminalBonus = terminalBonus == null ? ""
                : terminalBonus.stripTrailingZeros().toPlainString();
        } else if (AmountType.percenta.getCode().equals(amountType)) {
            this.terminalBonus = terminalBonus.multiply(new BigDecimal(100)).stripTrailingZeros()
                .toPlainString() + "%";
        }
    }
    ...
    public void setDirectBonus(BigDecimal directBonus, Integer amountType) {
        if (AmountType.amount.getCode().equals(amountType)) {
            this.directBonus = directBonus == null ? ""
                : directBonus.stripTrailingZeros().toPlainString();
        } else if (AmountType.percenta.getCode().equals(amountType)) {
            this.directBonus = directBonus.multiply(new BigDecimal(100)).stripTrailingZeros()
                .toPlainString() + "%";
        }
    }
    ...
    public void setIndirectBonus(BigDecimal indirectBonus, Integer amountType) {
        if (AmountType.amount.getCode().equals(amountType)) {
            this.indirectBonus = indirectBonus == null ? ""
                : indirectBonus.stripTrailingZeros().toPlainString();
        } else if (AmountType.percenta.getCode().equals(amountType)) {
            this.indirectBonus = indirectBonus.multiply(new BigDecimal(100)).stripTrailingZeros()
                .toPlainString() + "%";
        }
    }
    ...
    public String toString() {
        return "FinancingProductDetailVO{" + "id=" + id + ", productName='" + productName + '\''
               + ", redirectUrl='" + redirectUrl + '\'' + ", terminalBonus='" + terminalBonus + '\''
               + ", directBonus='" + directBonus + '\'' + ", indirectBonus='" + indirectBonus + '\''
               + ", logoUrl='" + logoUrl + '\'' + ", cashbackDate='" + cashbackDate + '\''
               + ", level='" + level + '\'' + ", backgroundStrength='" + backgroundStrength + '\''
               + ", riskControl='" + riskControl + '\'' + ", operationCapability='"
               + operationCapability + '\'' + ", startAmount='" + startAmount + '\''
               + ", startPeriod='" + startPeriod + '\'' + ", rebackName='" + rebackName + '\''
               + ", rebackValue='" + rebackValue + '\'' + ", totalReturn='" + totalReturn + '\''
               + ", aveRevenue='" + aveRevenue + '\'' + ", cashbackRule='" + cashbackRule + '\''
