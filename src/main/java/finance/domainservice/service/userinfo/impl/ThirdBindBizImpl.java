...
public class ThirdBindBizImpl implements ThirdBindBiz {
        ...
        if (org.apache.commons.lang3.StringUtils.isNotBlank(openId)) {
            ...
            if (CollectionUtils.isNotEmpty(inviteOpenInfoList)) {
                ...
                if (Objects.nonNull(parentUserInfo)) {
                    ...
                    List<FinanceUserInviteInfo> inviteInfoList = inviteInfoMapper.query(parameters);
                    if (CollectionUtils.isEmpty(inviteInfoList)) {
                        // 查询user_id 邀请关系是否已经建立
                        Map<String, Object> queryParameters = Maps.newHashMap();
                        queryParameters.put("userId", userId);
                        List<FinanceUserInviteInfo> inviteInfos = inviteInfoMapper
                        ...
                        log.info("parentUserId:{},userId:{}邀请关系已经存在!", parentUserInfo.getId(),
                            userId);
                    }
                } else {
    ...
    @Override
    public ResponseResult<Map<String, String>> queryOpenId(String channel, String code,
                                                           String qqOpenId) {
        // 获取 OpenId
