 ...
 * 
 * @author lili
 * @version $Id: RegisterSendMessageServiceImpl.java, v0.1 2018/10/24 3:38 PM
 *          lili Exp $
 */
@Slf4j
...
public class RegisterSendMessageServiceImpl implements RegisterSendMessageService {
	...
	public void sendMessage(Long userId) {
		List<UserInfo> userInfoList = userInfoRepository.queryByCondition(Lists.newArrayList(userId));
	 ...
	 * @param userInfo
	 *            用户信息
	 * @param parentThirdAccountInfo
	 *            父级用户绑定信息
	 */
	private void sendFirstInviteMessage(UserInfo userInfo, ThirdAccountInfo parentThirdAccountInfo) {
		// 微信消息模版
		...
		weiXinTemplateMessageSendService.send(userInfo, parentThirdAccountInfo, weiXinMessageTemplate,
				Maps.newHashMap());
	}
	...
	private void sendSecondInviteMessage(UserInfo userInfo, ThirdAccountInfo parentThirdAccountInfo) {
		...
		weiXinTemplateMessageSendService.send(userInfo, grandThirdAccountInfo, weiXinMessageTemplate,
				Maps.newHashMap());
	...
	public void sendRewardMessage(Long userId, BigDecimal rewardAmount) {
		...
		weiXinTemplateMessageSendService.send(userInfo, thirdAccountInfo, weiXinMessageTemplate, parameters);
