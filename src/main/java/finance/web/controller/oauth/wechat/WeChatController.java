...
public class WeChatController {
	...
	public ResponseResult<WeChatCreateQrResponse> createTempQr(@Param("activityCode") String activityCode) {
		...
		try {
			...
			WeCharQrInfo weCharQrInfo = weChatPubQrService.createTempQr(activityCode, userInfo.getInviteCode());
	...
	public ResponseResult<UserActivityWeChatPubInfoQueryResponse> getWeChatPubInfo(
		...
		try {
			...
			Preconditions.checkArgument(StringUtils.isNotBlank(userInfo.getInviteCode()));
			/*
			 * UserActivityWeChatPubInfo userActivityWeChatPubInfo =
			 * userActivityWeChatPubInfoQueryService .query(activityCode,
			 * userInfo.getInviteCode());
			 */
			Long unsubscribeNum = userActivityWeChatPubInfoQueryService.queryUnsubscribeNum(activityCode,
					userInfo.getInviteCode(), userInfo.getId());
			...
			queryResponse.setHistorySubscribeNum(Objects.nonNull(firstInviteNum) ? firstInviteNum : 0L);
