...
public class BarrageMessageRepositoryImpl implements BarrageMessageRepository {
    ...
    @Resource
    private FinanceBarrageMessageDAO financeBarrageMessageMapper;
    ...
    public Page<BarrageMessage> query(BarrageMessageQueryCondition condition) {
        ...
        if (count > 0) {
            ...
            parameters.put("page", page);
            List<FinanceBarrageMessage> financeBarrageMessageList = financeBarrageMessageMapper
                .query(parameters);
            if (CollectionUtils.isNotEmpty(financeBarrageMessageList)) {
                page.setDataList(BarrageMessageConverter.convert2List(financeBarrageMessageList));
            }
        }
        return page;
    }
