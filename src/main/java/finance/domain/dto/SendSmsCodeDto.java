...
public class SendSmsCodeDto {
    ...
    public Boolean validateParam() {
        // 公共验证
        if (StringUtils.isEmpty(mobileNum) || StringUtils.isEmpty(useType)
            || !SmsUseType.contains(useType)) {
            return false;
        }
        // 不同的登录类型做不同的验证
        if (!SmsUseType.simpleRegist.toString().equals(useType)
            && (StringUtils.isEmpty(imgCodeId) || imgCodeId.length() != 32
                || StringUtils.isEmpty(imgCode) || imgCode.length() != Constant.imgcode_length)) {
            return false;
        }
        return true;
    }
    ...
    public String toString() {
        return "SendSmsCodeDto{" + "mobileNum='" + mobileNum + '\'' + ", useType='" + useType + '\''
               + ", platformCode='" + platformCode + '\'' + ", imgCodeId='" + imgCodeId + '\''
               + ", imgCode='" + imgCode + '\'' + '}';
    }
