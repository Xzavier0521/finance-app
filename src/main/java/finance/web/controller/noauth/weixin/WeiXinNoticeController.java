...
public class WeiXinNoticeController {
	...
	public void noticeGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		// 微信加密签名
				...
				switch (wxData.get("Event")) {
					case WeChatConstant.EVENT_TYPE_SUBSCRIBE :
						...
						CompletableFuture.supplyAsync(() -> {
							...
							weChatDataSynchronizeService.process(fromUserName, OperatorTypeEnum.ADD);
							return true;
						}, threadPoolExecutor);
						outputStreamWrite(outputStream, buildTextMessage(wxData, MessageUtils.welcomeText()));
						break;
					case WeChatConstant.EVENT_TYPE_UNSUBSCRIBE :
						...
						// 异步执行，防止回复微信服务器超时
						CompletableFuture.supplyAsync(() -> {
							weChatDataSynchronizeService.process(fromUserName, OperatorTypeEnum.DEL);
							return true;
						}, threadPoolExecutor);
						break;
		...
		switch (wxData.get("EventKey")) {
			...
			case WeChatConstant.JINRONGJIA_INTRODUCTION :
				...
				item graphicItem = item.builder().Title(WeChatConstant.WE_CHAT_PUB_TITLE)
						.Description(WeChatConstant.WE_CHAT_PUB_DESCRIPTION).PicUrl(WeChatConstant.WE_CHAT_PUB_PIC_URL)
						.Url(WeChatConstant.WE_CHAT_PUB_URL).build();
	...
	private String buildGraphicMessage(Map<String, String> wxData, Integer articleCount, List<item> articles) {
