...
public class RedEnvelopeRainDataServiceImpl implements RedEnvelopeRainDataService {
	 ...
	 */
	@Override
	public BasicResponse localData(UserInfo userInfo, String activityCode, RedEnvelopeRainTimeCodeEnum timeCode,
			Long totalNum, BigDecimal totalAmount) {
		BasicResponse response = new BasicResponse();
		try {
			Integer activityDay = DateUtils.getCurrentDay(LocalDate.now());
			RedEnvelopeRainData redEnvelopeRainData = RedEnvelopeRainData.builder().activityCode(activityCode)
					.timeCode(timeCode).userId(userInfo.getId()).mobilePhone(userInfo.getMobileNum()).totalNum(totalNum)
					.totalAmount(totalAmount).activityDay(activityDay).build();
			ThirdAccountInfo thirdAccountInfo = thirdAccountInfoRepository.queryByCondition(userInfo.getId());
			// 启用事务处理
			response = transactionTemplate.execute(status -> {
				...
				// 记录金币奖励日志
				redEnvelopeRainRewardRepository.save(
						RedEnvelopeRainReward.builder().userId(userInfo.getId()).mobilePhone(userInfo.getMobileNum())
								.activityCode(activityCode).activityDay(String.valueOf(activityDay)).totalNum(totalNum)
								.totalAmount(totalAmount.longValue()).timeCode(timeCode)
								.rewardType(RewardTypeEnum.RED_ENVELOPE_RAIN).build());
				// 发送微信模版消息
				if (Objects.nonNull(thirdAccountInfo) && StringUtils.isNotBlank(thirdAccountInfo.getOpenId())) {
					// 查询微信消息模版
