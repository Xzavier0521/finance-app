...
/**
 * <p>
 * 弹幕消息：现阶段保存到数据库，后期可以考虑放入redis或者采用rabbitmq
 * </p>
 *
 * @author lili
 * @version 1.0: BarrageMessageRepositoryImpl.java, v 0.1 2018/9/29 上午10:07 lili
 *          Exp $
 */
@Slf4j
...
public class BarrageMessageRepositoryImpl implements BarrageMessageRepository {
	...
	public int save(BarrageMessage barrageMessage) {
		return financeBarrageMessageMapper.insertSelective(BarrageMessageConverter.convert(barrageMessage));
	}
	...
	public Page<BarrageMessage> query(BarrageMessageQueryCondition condition) {
		Page<BarrageMessage> page = new Page<>(condition.getPageSize(), (long) condition.getPageNum());
		...
		if (count > 0) {
			...
			parameters.put("page", page);
			List<BarrageMessageDO> barrageMessageDOList = financeBarrageMessageMapper.query(parameters);
