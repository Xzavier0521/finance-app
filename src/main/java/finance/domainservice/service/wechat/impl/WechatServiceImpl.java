...
import finance.domain.dto.WechatPubJsapiTicketDto;
...
public class WechatServiceImpl implements WechatService {
    ...
    public String getWechatPubAccessToken() {
        Long expireTime = stringRedisTemplate
            .getExpire(wechatPubAccessTokenCacheKeyPrefix + wechatPubAppId, TimeUnit.MINUTES);
        ...
        if (expireTime < 30) {
            ...
            if (accessTokenDto != null && StringUtils.isEmpty(accessTokenDto.getErrcode())) {
                ...
                stringRedisTemplate.opsForValue().set(
                    wechatPubAccessTokenCacheKeyPrefix + wechatPubAppId, accessToken,
            ...
            accessToken = stringRedisTemplate.opsForValue()
                .get(wechatPubAccessTokenCacheKeyPrefix + wechatPubAppId);
        }
        return accessToken;
    }
    ...
    public String getWechatPubJsapiTicket() {
        String jsapi_ticket;
        jsapi_ticket = stringRedisTemplate.opsForValue()
            .get(wechatPubJsapiTicketCacheKeyPrefix + wechatPubAppId);
        ...
        if (jsapiTicketDto != null && "0".equals(jsapiTicketDto.getErrcode())) {
            ...
            stringRedisTemplate.opsForValue().set(
                wechatPubJsapiTicketCacheKeyPrefix + wechatPubAppId, jsapi_ticket,
                wechatPubAccessTokenTimeoutMinutes, TimeUnit.MINUTES);
        } else {
