package cn.zhishush.finance.web.controller.noauth.book.bookbiz.impl;
...
public class BookBizImpl implements BookBiz {
	...
	public List<BookCommentVO> queryBookCommentList(String bookId) {
		...
		List<String> bookCommentList = stringRedisTemplate.opsForList().range(bookCommentRedisKey, 0, -1);
		if (bookCommentList != null && bookCommentList.size() > 0) {
			...
			for (String bookComment : bookCommentList) {
				...
				List<String> commentLikeList = stringRedisTemplate.opsForList().range(bookCommentLikeRedisKey, 0, -1);
	...
	public Boolean addBookComment(XMap paramMap) {
		...
		/** 1.jwt 获取用户user_id **/
		UserInfoDO financeUserInfo = financeUserInfoMapper.selectByPrimaryKey(USER_ID);
