 ...
 * <p>注释</p>
 * @author lili
 * @version $Id: UserActivityWeChatPubInfoQueryServiceImpl.java, v0.1 2018/11/7 4:39 PM lili Exp $
 */
@Slf4j
@Service("userActivityWeChatPubInfoQueryService")
public class UserActivityWeChatPubInfoQueryServiceImpl implements
                                                       UserActivityWeChatPubInfoQueryService {
    ...
    public UserActivityWeChatPubInfo query(String activityCode, String inviteCode) {
        ...
        WeChatSubscribeInfo weChatSubscribeInfo = inviteOpenInfoRepository
            .countSubscribeInfo(inviteCode);
        userActivityWeChatPubInfo.setActivityCode(activityCode);
        if (Objects.nonNull(weChatSubscribeInfo)) {
            userActivityWeChatPubInfo.setHistorySubscribeNum(
                Objects.nonNull(weChatSubscribeInfo.getHistorySubscribeNum())
                    ? weChatSubscribeInfo.getHistorySubscribeNum()
                    : 0L);
            userActivityWeChatPubInfo
                .setSubscribeNum(Objects.nonNull(weChatSubscribeInfo.getSubscribeNum())
                    ? weChatSubscribeInfo.getSubscribeNum()
                    : 0L);
            userActivityWeChatPubInfo
                .setUnsubscribeNum(Objects.nonNull(weChatSubscribeInfo.getUnsubscribeNum())
