...
public class SignatureAspect {
	...
	public Boolean before(JoinPoint joinPoint) throws SignatureException {
		...
		if ("/monitor".equals(path) || "/systemCurrentTime".equals(path) || path.startsWith("/book/") // XXX 骗审接口跳过
		) {
			// 跳过验签
		...
		String paramStr = "";
		/*
		 * 验签说明： 1.当前的入参中不会有数组； 2.入参的json只有一级； 3.如果入参 json 需要多级，则下一级的数据作为一个整体的jsonString
		 * 来传递，不是json格式；
		 */
		...
		if (StringUtils.isEmpty(timestampStr) || StringUtils.isEmpty(noncestr) || StringUtils.isEmpty(signature)
		...
		stringRedisTemplate.opsForValue().set(signatureCacheKey, "", signatureTimeout, TimeUnit.SECONDS);
