...
public class LeaderBoardQueryServiceImpl implements LeaderBoardQueryService {
	...
	public List<LeaderBoard> queryByType(LeaderBoardTypeEnum leaderBoardType, String activityCode, int num) {
		...
		String key = MessageFormat.format("{0}:{1}:{2}", RedEnvelopConstant.LEADER_BOARD, activityCode,
		...
		// 排行榜的key
		Set<String> leaderBoardKeys = leadBoardSet.stream().map(v -> (String) v).collect(Collectors.toSet());
		HashOperations<String, Object, Object> hashOperations = redisTemplate.opsForHash();
		for (String leadBoardKey : leaderBoardKeys) {
			Map<String, Object> fieldMap = Maps.newHashMap();
			LeaderBoard.fieldSet().forEach(field -> fieldMap.put(field, hashOperations.get(leadBoardKey, field)));
	 ...
	 */
	@Override
	public LeaderBoard queryCurrentLeaderBoard(UserInfo userInfo, LeaderBoardTypeEnum leaderBoardType) {
		...
		switch (leaderBoardType) {
			...
			case SECOND_LEVEL :
				...
				Integer inviteNumbers = (Integer) hashOperations.get(inviteNumbersKey, userInfo.getId());
