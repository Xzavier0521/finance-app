...
import finance.ext.service.weixin.WeiXinUserInfoQueryService;
 ...
 *
 * @author lili
 * @version $Id: WeiXinUserInfoQueryClientImpl.java, v0.1 2018/10/23 3:08 PM
 *          lili Exp $
 */
@Slf4j
...
public class WeiXinUserInfoQueryClientImpl implements WeiXinUserInfoQueryClient {
	...
	public UserInfoQueryResponse queryUserList(String accessToken, String nextOpenid) {
		...
		try {
			...
			response = create().queryUserList(accessToken, StringUtils.isNotBlank(nextOpenid) ? nextOpenid : "")
					.execute().body();
		...
		log.info("[结束获取用户列表],请求参数:accessToken:{},nextOpenid:{},返回结果:{}", accessToken, nextOpenid, response);
		return response;
	}
	...
	public WeiXinUserInfoDetail queryUserInfo(String accessToken, String openId) {
		...
		log.info("[结束获取用户基本信息],请求参数:accessToken:{},nextOpenid:{},返回结果:{}", accessToken, openId, response);
		return response;
	}
	...
	public WeiXinBatchQueryUserInfoResponse batchQueryUserInfo(WeiXinBatchQueryUserInfoRequest request) {
		...
		try {
			response = create().batchQueryUserInfo(request.getAccessToken(), request).execute().body();
