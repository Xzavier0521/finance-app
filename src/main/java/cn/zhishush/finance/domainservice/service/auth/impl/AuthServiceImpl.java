package cn.zhishush.finance.domainservice.service.auth.impl;
...
import cn.zhishush.finance.ext.integration.datapay.DataPayClient;
...
public class AuthServiceImpl implements AuthService {
	...
	public ResponseResult<String> auth(UserBankCardDto bankCardDto) {
		...
		if (!"1".equals(authSwitch)) {
			logger.info(LogUtil.getFormatLog("bank.card.auth.switch=" + authSwitch, "银行卡认证开关关闭，不发送验证请求"));
		...
		if (authResDto != null && Constant.authSucc.equals(authResDto.getCode())) {
			...
			logger.error(LogUtil.getFormatLog(authResDto != null ? authResDto.getMessage() : null, "银行卡验证失败"), "");
			return ResponseResult.error(CodeEnum.bankCardAuthError);
		}
		return ResponseResult.success(null);
	}
