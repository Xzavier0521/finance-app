...
public class DateUtil {
    ...
    public static Date curRandHourMinSec(Date date) {
        ...
        hour = rand.nextInt(24 - hour) + hour;
        min = rand.nextInt(60 - min) + min;
        sec = rand.nextInt(60 - sec) + sec;
     ...
     * @Description: 随机生成时间，范围：date-24：00
     */
    public static Date cumsDate(int hmax, int hmin, int mmax, int mmin, int smax, int smin,
                                Date date) {
        Random rand = new Random();
        int h = rand.nextInt(hmax - hmin) + hmin;
        if (h >= 20) {
            h = 20;
            mmax = 30;
            mmin = 0;
        }
        int m = rand.nextInt(mmax - mmin) + mmin;
        int s = rand.nextInt(smax - smin) + smin;
