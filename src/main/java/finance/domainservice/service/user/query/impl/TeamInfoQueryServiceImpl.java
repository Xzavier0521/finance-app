...
public class TeamInfoQueryServiceImpl implements TeamInfoQueryService {
     ...
     */
    @Override
    public TeamInfoQueryResult queryTeamUserList(Long userId, CustomerQueryTypeEnum type,
                                                 int maxCount) {
        ...
        //  查询明细
        List<FinanceUserInviteInfo> firstLevelInviteInfoList = userInviteRepository
            .queryByCondition(1, maxCount, userId).getDataList();
        // 一级用户id列表
        List<Long> firstLevelUserIdList = firstLevelInviteInfoList.stream()
            .map(FinanceUserInviteInfo::getUserId).collect(Collectors.toList());
        List<SecondLevelTeamUserInfo> firstLevelUserInfoList = buildTeamUserInfo(
            firstLevelInviteInfoList);
        ...
        if (secondLevelUserCount == 0) {
            ...
            List<FirstLevelTeamUserInfo> allTeamUserInfoList = Lists
                .newArrayListWithCapacity(firstLevelUserCount);
        ...
        //  查询明细
        List<FinanceUserInviteInfo> secondLevelInviteInfoList = userInviteRepository
            .querySecondLevelInviteUser(userId, 1, maxCount).getDataList();
        // 二级用户id列表
        List<SecondLevelTeamUserInfo> secondLevelUserInfoList = buildTeamUserInfo(
            secondLevelInviteInfoList);
        ...
        if (CustomerQueryTypeEnum.ALl == type) {
            ...
            Page<FinanceUserInviteInfo> userInviteInfoPage = userInviteRepository
                .queryByCondition(1, 300, parentUserIds);
            List<SecondLevelTeamUserInfo> secondTeamUserInfoList = Lists.newArrayList();
            if (userInviteInfoPage.getTotalCount() > 0) {
                List<FinanceUserInviteInfo> financeUserInviteInfoList = userInviteInfoPage
                    .getDataList();
            ...
            for (SecondLevelTeamUserInfo firstTeamUserInfo : firstLevelUserInfoList) {
                ...
                List<SecondLevelTeamUserInfo> secondLevelTeamUserInfoList = secondTeamUserInfoList
                    .stream().filter(secondLevelTeamUserInfo -> secondLevelTeamUserInfo
                        .getParentUserId().equals(firstTeamUserInfo.getUserId()))
    ...
    private List<SecondLevelTeamUserInfo> buildTeamUserInfo(List<FinanceUserInviteInfo> userInviteInfoList) {
        List<Long> userIdList = userInviteInfoList.stream().map(FinanceUserInviteInfo::getUserId)
            .collect(Collectors.toList());
        List<SecondLevelTeamUserInfo> teamUserInfoList = Lists
            .newArrayListWithCapacity(userIdList.size());
        // 用户信息
        ...
        userInfoList.forEach(userInfo -> {
            List<CardInfo> cardInfos = cardInfoList.stream()
                .filter(cardInfo -> cardInfo.getId().equals(userInfo.getId()))
                .collect(Collectors.toList());
            String userName = "***";
            ...
            FinanceUserInviteInfo userInviteInfo = userInviteInfoList.stream().filter(
                financeUserInviteInfo -> financeUserInviteInfo.getUserId().equals(userInfo.getId()))
